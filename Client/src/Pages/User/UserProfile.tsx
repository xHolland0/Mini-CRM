...
function UserProfilePage() {
  ...
  useEffect(() => {
    const fetchBackendUser = async () => {
      if (isAuthenticated && auth0User?.sub) {
        ...
        setApiError(null);
        try {
          // Kendi API'mizdeki SyncUser endpoint'ini çağırarak
          // kullanıcının veritabanı kaydını al (veya oluştur)
  ...
  // Kendi API'mizden gelen bilgiler
  ...
  return (
    <Box sx={{ mt: 4, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
      ...
      <Paper elevation={3} sx={{ p: 4, borderRadius: 2, maxWidth: 500, width: '100%', textAlign: 'center' }}>
        <Avatar 
          alt={displayName} 
          src={displayPicture} 
